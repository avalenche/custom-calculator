...
The primary objective was to implement a functional scientific calculator using pure JavaScript while adhering to strict technical constraints, including:

No external frameworks or libraries (e.g., React, jQuery, or Math.js).

All complex mathematical functions (e.g., power, roots, factorial) must be custom implemented.

Implementation of the Command Pattern for handling operations and button presses.

Full exception handling (e.g., division by zero).

Unit testing for all mathematical operations using Jest.

Webpack configuration to produce an optimized build output of only one HTML and one JS file.

2. How to run the app

This application uses Webpack for optimized bundling and requires Node.js and npm to run the build scripts.
...
Install all dependencies (Webpack, ESLint, Jest, etc.):
npm install
...
Production Build,
npm run build,
"Creates the optimized, production-ready files in the dist/ folder."
Run Tests,
npm test,
Executes all unit tests defined in src/math.test.js.
Code Check,
npm run lint,
Runs ESLint to check code quality and style compliance.

This project implements a custom scientific calculator using pure JavaScript, strictly adhering to all functional and technical requirements.
